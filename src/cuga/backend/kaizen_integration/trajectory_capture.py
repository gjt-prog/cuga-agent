...
class TrajectoryCapture:
    ...
    def add_tool_call(self, tool_name: str, args: Dict[str, Any], result: Any):
        ...
        self.add_assistant_message(
            tool_call_content, metadata={"type": "tool_call", "tool": tool_name, "args": args}
        )

        # Add tool result as system message
        result_content = f"Tool Result: {str(result)[:500]}"  # Limit result length
        self.add_system_message(result_content, metadata={"type": "tool_result", "tool": tool_name})
    ...
    def summarize(self) -> str:
        ...
        tool_calls = sum(1 for m in self.messages if m.get("metadata", {}).get("type") == "tool_call")

        code_execs = sum(1 for m in self.messages if m.get("metadata", {}).get("type") == "code_execution")
