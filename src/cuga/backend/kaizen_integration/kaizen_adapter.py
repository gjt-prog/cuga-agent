...
class KaizenAdapter:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Kaizen adapter initialized successfully (namespace: {self.namespace_id}, provider: {self.provider})"
            )
    ...
    def get_guidelines(self, task: str, limit: int = 5) -> List[str]:
        ...
        try:
            ...
            results = self.client.search_entities(
                namespace_id=self.namespace_id, query=task, filters={"type": "guideline"}, limit=limit
            )
    ...
    def save_trajectory(
        self, messages: List[Dict[str, Any]], task_id: Optional[str] = None, generate_tips: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            self.client.update_entities(
                namespace_id=self.namespace_id, entities=entities, enable_conflict_resolution=False
            )
            ...
            if generate_tips:
                ...
                if tips:
                    ...
                    self.client.update_entities(
                        namespace_id=self.namespace_id, entities=tip_entities, enable_conflict_resolution=True
                    )
    ...
    def add_guideline(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            entity = self.Entity(type='guideline', content=content, metadata=metadata or {})

            self.client.update_entities(
                namespace_id=self.namespace_id, entities=[entity], enable_conflict_resolution=True
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            guidelines = self.client.search_entities(
                namespace_id=self.namespace_id, filters={"type": "guideline"}, limit=1000
            )

            trajectories = self.client.search_entities(
                namespace_id=self.namespace_id, filters={"type": "trajectory"}, limit=1000
            )
