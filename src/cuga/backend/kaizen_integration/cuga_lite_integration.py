...
class CugaLiteKaizenIntegration:
    ...
    def capture_step(self, step_type: str, content: str, metadata: Optional[Dict[str, Any]] = None):
        """
    ...
    def save_trajectory(
        self, final_answer: Optional[str] = None, success: bool = True, state: Optional[AgentState] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if final_answer:
                self.trajectory.add_assistant_message(
                    final_answer, metadata={"type": "final_answer", "success": success}
                )
            ...
            saved = self.adapter.save_trajectory(
                messages=messages, task_id=task_id, generate_tips=generate_tips
            )
